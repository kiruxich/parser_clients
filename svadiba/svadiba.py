...
async def main():
    ...
    city = "moscow"
    file_path = os.path.join(BASE_DIR, "свадебные_агентства.xlsx")
    progress_file = os.path.join(BASE_DIR, "progress_wedding.txt")
    ...
    MAX_PAGES_PER_CELL = 10  # Охват до 10 страниц для плотных секторов (ЦАО)
    CONCURRENCY_LIMIT = 4
    ...
    async with async_playwright() as p:
        browser = await p.chromium.launch(
            headless=False, # Видимый режим браузера
        ...
        try:
            for q_idx, search_query in enumerate(search_queries, 1):
                ...
                for cell_idx, (lat, lon, sector_name) in enumerate(grid_points, 1):
                    ...
                    # Извлекаем короткое имя для консоли. Например: "Сектор A1"
                    ...
                    for page_num in range(1, MAX_PAGES_PER_CELL + 1):
                        ...
                        try:
                            ...
                            try:
                                ...
                                await first_card.hover() # Наводим мышь на первую карточку
                            except Exception:
