...
async def main():
    ...
    city = "moscow"
    file_path = os.path.join(BASE_DIR, "юридические_услуги.xlsx")
    progress_file = os.path.join(BASE_DIR, "progress_yur.txt")
    ...
    if os.path.exists(file_path):
        ...
        wb.save(file_path) # Сохраняем мгновенно
        results_count = 0
    ...
    async with async_playwright() as p:
        browser = await p.chromium.launch(
            headless=False,  # Открываем браузер, чтобы можно было решить капчу вручную
