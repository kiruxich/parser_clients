...
async def process_firm(context, firm_id, url, ws, wb, file_path, lock, semaphore, state):
    ...
    async with semaphore:
        ...
        try:
            ...
            good_indicators = [".ru", ".com", ".рф", ".org", ".net", ".info", ".su", ".pro", ".moscow", ".club", ".space", ".io", ".me", ".kz", ".by", "vk.com", "t.me", "wa.me", "taplink.cc", "instagram.com"]
        ...
        try:
            for q_idx, search_query in enumerate(search_queries, 1):
                ...
                for cell_idx, (lat, lon, sector_name) in enumerate(grid_points, 1):
                    # Вывод теперь супер понятный, например: 
                    # 📍 [кальянная] [1/25]: Сектор A1 (Куркино, Сев. Тушино)
